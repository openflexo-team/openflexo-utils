...
public class StateDocx {
	...
	private void init(WordMLDocument doc) {
		...
		this.wordMLPackage = ((DocumentML) root.getElementML()).getWordprocessingMLPackage();

		if (Util.getCustomDocumentProperty(wordMLPackage.getDocPropsCustomPart(), CustomProperties.PROMPT_FOR_CHECKIN_MESSAGE)
				.equals("true")) {
			this.promptForCheckinMessage = Boolean.TRUE;

		}
		else if (Util.getCustomDocumentProperty(wordMLPackage.getDocPropsCustomPart(), CustomProperties.PROMPT_FOR_CHECKIN_MESSAGE)
				.equals("false")) {
		...
		this.chunkingStrategy = Util.getCustomDocumentProperty(wordMLPackage.getDocPropsCustomPart(), CustomProperties.CHUNKING_STRATEGY);
		...
		while (partsIterator.hasNext()) {
			...
			if (pairs.getValue() instanceof CustomXmlDataStoragePart) {
				CustomXmlDataStoragePart docx4jPart = (CustomXmlDataStoragePart) pairs.getValue();
	...
	public class TransformsCollection {
		...
		private int tSequenceNumberAtLoadTime = Integer.parseInt(
				Util.getCustomDocumentProperty(wordMLPackage.getDocPropsCustomPart(), CustomProperties.DOCUMENT_TRANSFORM_SEQUENCENUMBER));
