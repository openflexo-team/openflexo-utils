...
public class Part {
	...
	public static Part factory(org.docx4j.openpackaging.parts.JaxbXmlPart docx4jPart) {
		...
		if (docx4jPart instanceof org.docx4j.openpackaging.parts.relationships.RelationshipsPart) {
			...
			partXml = org.docx4j.XmlUtils.marshaltoString(docx4jPart.getJaxbElement(), true, Context.jcRelationships);
		}
		else {
			partXml = org.docx4j.XmlUtils.marshaltoString(docx4jPart.getJaxbElement(), true); // suppressDeclaration
		}

		return factory(FlatOpcXmlCreator.wrapInXmlPart(partXml, docx4jPart.getPartName().getName(), docx4jPart.getContentType()));
	...
	public static Part factory(String partXml) {
		...
		org.w3c.dom.Document doc = null;
		try {
			doc = dbf.newDocumentBuilder().parse(new InputSource(new StringReader(partXml)));
		} catch (Exception e) {
		...
		Node xmlNode = doc.getDocumentElement(); // .getFirstChild();
		...
		String partName = xmlNode.getAttributes().getNamedItemNS(Namespaces.PKG_XML, "name").getNodeValue();
		...
		else if (partName.equals("/word/footnotes.xml") | partName.equals("/word/endnotes.xml") | partName.equals("/word/comments.xml")) {
	...
	public void init(org.w3c.dom.Document doc) {
		...
		contentType = xmlNode.getAttributes().getNamedItemNS(Namespaces.PKG_XML, "contentType").getNodeValue();
