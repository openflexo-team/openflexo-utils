...
public class ParagraphPropertiesML extends ElementML implements PropertiesContainerML {
	...
	public ParagraphPropertiesML(PPr pPr, ElementMLFactory elementMLFactory) {
		super(pPr, elementMLFactory, false);
	}
	...
	public void save() {
		...
		org.docx4j.wml.PPr pPr = (org.docx4j.wml.PPr) this.docxObject;

		// ALIGNMENT attribute
		if (this.attrs.isDefined(StyleConstants.Alignment)) {
			Integer align = (Integer) this.attrs.getAttribute(StyleConstants.Alignment);
			org.docx4j.wml.Jc jc = getObjectFactory().createJc(align);
			if (jc != null) {
				pPr.setJc(jc);
			}
		} else {
		...
		if (this.attrs.isDefined(WordMLStyleConstants.PStyleAttribute)) {
			String pStyle = (String) this.attrs.getAttribute(WordMLStyleConstants.PStyleAttribute);
			if (pPr.getPStyle() == null) {
				pPr.setPStyle(getObjectFactory().createPStyle(pStyle));
			} else {
	...
	public Object clone() {
		...
		return new ParagraphPropertiesML(obj, getElementMLFactory());
	}
