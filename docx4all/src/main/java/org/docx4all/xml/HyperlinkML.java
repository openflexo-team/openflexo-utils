...
public class HyperlinkML extends ElementML {
	...
	public HyperlinkML(Object docxObject, ElementMLFactory elementMLFactory) {
		this(docxObject, elementMLFactory, false);
		this.dummyTarget = null;
	}

	public HyperlinkML(Object docxObject, ElementMLFactory elementMLFactory, boolean isDummy) {
	...
	public String getTarget() {
		...
		org.docx4j.openpackaging.packages.WordprocessingMLPackage wmlPkg = getWordprocessingMLPackage();
		if (wmlPkg != null && wmlPkg.getMainDocumentPart() != null) {
			org.docx4j.openpackaging.parts.relationships.RelationshipsPart part = wmlPkg.getMainDocumentPart().getRelationshipsPart();
			if (part != null) {
	...
	public void setDisplayText(String s) {
		...
		if (runML == null) {
			runML = new RunML(getObjectFactory().createR(s), getElementMLFactory());
			...
			if (ml == null) {
				ml = new RunContentML(getObjectFactory().createT(s), getElementMLFactory());
				runML.addChild(ml);
			} else {
				ml.setTextContent(s);
			}
	...
	private RunPropertiesML createRunPropertiesML() {
		...
		return getElementMLFactory().createRunPropertiesML(attrs);
	}
	...
	public boolean canAddSibling(ElementML elem, boolean after) {
		...
		if (elem instanceof RunML || elem instanceof RunDelML || elem instanceof RunDelML || elem instanceof HyperlinkML) {
	...
	public Object clone() {
		...
		return new HyperlinkML(obj, getElementMLFactory(), this.isDummy);
	}
	...
	private void initChildren(org.docx4j.wml.P.Hyperlink link) {
		...
		if (!pKids.isEmpty()) {
			...
			for (Object o : pKids) {
				RunML run = new RunML(o, getElementMLFactory(), this.isDummy);
