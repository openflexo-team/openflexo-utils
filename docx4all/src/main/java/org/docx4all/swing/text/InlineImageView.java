...
/**
 * This class is based on javax.swing.text.html.ImageView class downloaded from <a href="http://download.java.net/openjdk/jdk7/">OpenJDK
 * Source Releases</a>
 */
public class InlineImageView extends View implements PropertyChangeListener {
	/**
	 * If true, when some of the bits are available a repaint is done.
	 * <p>
	 * This is set to false as swing does not offer a repaint that takes a delay. If this were true, a bunch of immediate repaints would get
	 * generated that end up significantly delaying the loading of the image (or anything else going on for that matter).
	 */
	...
	private int height;
	/**
	 * Bitmask containing some of the above bitmask values. Because the image loading notification can happen on another thread access to
	...
	public InlineImageView(Element elem) {
		...
		if (elem.getParentElement() instanceof DocumentElement) {
			((DocumentElement) elem.getParentElement()).getPropertyChangeSupport().addPropertyChangeListener(this);
		}
	...
	/**
	 * Sets how the image is loaded. If <code>newValue</code> is true, the image we be loaded when first asked for, otherwise it will be
	 * loaded asynchronously. The default is to not load synchronously, that is to load the image asynchronously.
	 */
	...
	/**
	 * For images the tooltip text comes from text specified with the <code>ALT</code> attribute. This is overriden to return
	...
	protected void setPropertiesFromAttributes() {
		...
		if (getElement() instanceof DocumentElement) {
			...
			if (getElement().getParentElement() instanceof DocumentElement) {
				if (((DocumentElement) getElement()).getSelected()) {
					borderColor = Color.RED;
					backgroundColor = WordMLDocument.SELECTION_COLOR;
					borderSize = 2;
				}
				else {
					borderColor = null;
	...
	private void paintBackground(Graphics g, Rectangle rect) {
		...
		if (backgroundColor != null) {
			...
			g.fillRect(rect.x + xOffset, rect.y + yOffset, rect.width - xOffset - xOffset - 1, rect.height - yOffset - yOffset - 1);
		}
	}
	 ...
	 * @return the span the view would like to be rendered into; typically the view is told to render into the span that is returned,
	...
	/**
	 * Determines the desired alignment for this view along an axis. This is implemented to give the alignment to the bottom of the icon
	 * along the y axis, and the default along the x axis.
	 *
	 * @param axis
	 *            may be either X_AXIS or Y_AXIS
	 * @return the desired alignment; this should be a value between 0.0 and 1.0 where 0 indicates alignment at the origin and 1.0 indicates
	...
	/**
	 * Loads the image and updates the size accordingly. This should be invoked instead of invoking <code>loadImage</code> or
	...
	private void loadImage() {
		WordprocessingMLPackage wmlPkg = getInlineDrawingML().getWordprocessingMLPackage();
	...
	private class ImageHandler implements ImageObserver {
		...
		public boolean imageUpdate(Image img, int flags, int x, int y, int newWidth, int newHeight) {
			...
			if ((flags & ImageObserver.HEIGHT) != 0 && extent.height != newHeight) {
				changed |= 1;
			}
			if ((flags & ImageObserver.WIDTH) != 0 && extent.width != newWidth) {
				changed |= 2;
			}
	...
	/**
	 * ImageLabelView is used if the image can't be loaded, and the attribute specified an alt attribute. It overriden a handle of methods
	 * as the text is hardcoded and does not come from the document.
	 */
