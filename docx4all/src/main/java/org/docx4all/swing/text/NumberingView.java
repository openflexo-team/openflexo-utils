...
public class NumberingView extends View {
	...
	private void drawBullet(Graphics g, int ax, int ay, int aw, int ah, float align) {
	...
	private void drawNumberingString(Graphics g, int ax, int ay, int aw, int ah, float align) {
		...
		if (this.numbering != null) {
			SwingUtilities2.drawString((JComponent) getContainer(), g, this.numbering.getNumString(), x, y);
		}
		g.setFont(origFont);
	}
	 ...
	 * @return the span the view would like to be rendered into >= 0. Typically the view is told to render into the span that is returned,
	 ...
	 *            the allocated region to render into
	 * @param biasReturn
	 *            either <code>Position.Bias.Forward</code> or <code>Position.Bias.Backward</code> is returned as the zero-th element of
	 *            this array
	...
	/**
	 * Determines how attractive a break opportunity in this view is. This can be used for determining which view is the most attractive to
	 * call <code>breakView</code> on in the process of formatting. A view that represents text that has whitespace in it might be more
	 * attractive than a view that has no whitespace, for example. The higher the weight, the more attractive the break. A value equal to or
	 * lower than <code>BadBreakWeight</code> should not be considered for a break. A value greater than or equal to
	 * <code>ForcedBreakWeight</code> should be broken.
	 * <p>
	 * This is implemented to provide the default behavior of returning <code>BadBreakWeight</code> unless the length is greater than the
	 * length of the view in which case the entire view represents the fragment. Unless a view has been written to support breaking
	 * behavior, it is not attractive to try and break the view. An example of a view that does support breaking is <code>LabelView</code>.
	 * An example of a view that uses break weight is <code>ParagraphView</code>.
	 *
	 * @param axis
	 ...
	 * @param len
	 *            specifies the relative length from <em>pos</em> where a potential break is desired >= 0
	...
	/**
	 * Breaks this view on the given axis at the given length. This is implemented to attempt to break on a whitespace location, and returns
	 * a fragment with the whitespace at the end. If a whitespace location can't be found, the nearest character is used.
	 *
	 * @param axis
	 *            may be either View.X_AXIS or View.Y_AXIS
	 * @param p0
	...
	/**
	 * Creates a view that represents a portion of the element. This is potentially useful during formatting operations for taking
	 * measurements of fragments of the view. If the view doesn't support fragmenting (the default), it should return itself.
	 * <p>
	 * This view does support fragmenting. It is implemented to return a nested class that shares state in this view representing only a
	 * portion of the view.
	 *
	 * @param p0
	 *            the starting offset >= 0. This should be a value greater or equal to the element starting offset and less than the element
	 *            ending offset.
	 * @param p1
	 *            the ending offset > p0. This should be a value less than or equal to the elements end offset and greater than the elements
	 *            starting offset.
	...
	/**
	 * Provides a way to determine the next visually represented model location that one might place a caret. Some views may not be visible,
	 * they might not be in the same order found in the model, or they just might not allow access to some of the locations in the model.
	 *
	 * @param pos
	 *            the position to convert >= 0
	 * @param a
	 *            the allocated region to render into
	 * @param direction
	 *            the direction from the current position that can be thought of as the arrow keys typically found on a keyboard. This may
	 *            be SwingConstants.WEST, SwingConstants.EAST, SwingConstants.NORTH, or SwingConstants.SOUTH.
	...
	protected void setPropertiesFromAttributes() {
		AttributeSet attr = getElement().getParentElement().getAttributes();
		PPrBase.NumPr numPr = (PPrBase.NumPr) attr.getAttribute(WordMLStyleConstants.NumPrAttribute);
		ElementML elemML = (ElementML) attr.getAttribute(WordMLStyleConstants.ElementMLAttribute);
		String pStyle = (String) attr.getAttribute(WordMLStyleConstants.PStyleAttribute);
		String numId = null;
