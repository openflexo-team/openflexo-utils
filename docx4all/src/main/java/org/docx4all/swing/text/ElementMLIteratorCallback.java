...
public class ElementMLIteratorCallback extends ElementMLIterator.Callback {
	...
	private final List<ElementSpec> _elementSpecs = new ArrayList<ElementSpec>();
	...
	private String _fldComplexMode = ElementMLIteratorCallback.SHOW_FLD_COMPLEX_INSTR;
	private final Stack<FldComplexML> _fldComplexes = new Stack<FldComplexML>();
	private final Stack<ElementML> _ignoredElements = new Stack<ElementML>();

	public void setFldComplexMode(String mode) {
		_fldComplexMode = ElementMLIteratorCallback.SHOW_FLD_COMPLEX_INSTR.equals(mode) ? ElementMLIteratorCallback.SHOW_FLD_COMPLEX_INSTR
	...
	public void handleStartElement(ElementML elem) {
		...
		} else if (elem instanceof RunInsML || elem instanceof RunDelML || elem instanceof HyperlinkML) {
			;// bypass
	...
	private void addContentElementSpec(AttributeSet attrs, String text) {
		ElementSpec es = new ElementSpec(attrs, ElementSpec.ContentType, text.toCharArray(), 0, text.length());
		_elementSpecs.add(es);
	}
	...
	private void openElementSpec(ParagraphML paragraphML) {
		...
		if (pc != null) {
			...
			String pStyle = (String) pcAttrs.getAttribute(WordMLStyleConstants.PStyleAttribute);
			if (pStyle != null) {
				StyleSheet styleSheet = paragraphML.getStyleSheet();
				if (styleSheet != null) {
					Style s = styleSheet.getIDStyle(pStyle);
					if (s != null) {
						((SimpleAttributeSet) _paragraphAttrs).setResolveParent(s);
					}
				}
			}
		}
	...
	private void closeElementSpec(ParagraphML paragraphML, boolean resetAttr) {
		...
		RunContentML rcML = ElementML.IMPLIED_NEWLINE;
	...
	private void openElementSpec(RunML runML) {
		...
		if (fldChar != null) {
			...
			org.docx4j.wml.STFldCharType type = fldChar.getFldCharType();
			FldComplexML fldComplex;
			if (type == org.docx4j.wml.STFldCharType.BEGIN) {
				fldComplex = new FldComplexML(runML.getElementMLFactory());
				_fldComplexes.add(fldComplex);
				createSpec = (_fldComplexMode == ElementMLIteratorCallback.SHOW_FLD_COMPLEX_INSTR);
			...
			if (fldComplex.getSeparate() == null) {
				...
				createSpec = (_fldComplexMode == ElementMLIteratorCallback.SHOW_FLD_COMPLEX_VALUE);
			}
			fldComplex.addChild(runML);
		}
		...
		if (createSpec) {
			...
			if (pc != null) {
				...
				String rStyle = (String) pcAttrs.getAttribute(WordMLStyleConstants.RStyleAttribute);
				if (rStyle != null) {
