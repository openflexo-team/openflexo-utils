...
public class ToolBarStates extends InternalFrameAdapter implements FocusListener, DocumentListener, InputAttributeListener, FlavorListener,
	...
	/**
	 * The binding key used for this ToolBarStates object when passed into scripting environment
	 */
	...
	public void setDocumentDirty(JInternalFrame iframe, boolean dirty) {
		...
		if (iframe == null) {
			System.out.println("TODO: Check this: forcing notification because of null iframe");
			firePropertyChange(DOC_DIRTY_PROPERTY_NAME, !dirty, dirty);
			return;
		}

		if (log.isDebugEnabled()) {
			String file = (String) iframe.getClientProperty(WordMLDocument.FILE_PATH_PROPERTY);
			log.debug("setDocumentDirty(): File = " + VFSUtils.getFriendlyName(file) + " - 'dirty' parameter = " + dirty);
		}
	...
	public void setDocumentShared(boolean b) {
		...
		if (_isDocumentShared != null && _isDocumentShared.booleanValue() == b) {
			return;
		}
		...
		firePropertyChange(DOC_SHARED_PROPERTY_NAME, oldValue, _isDocumentShared);
	}
	...
	public void setLocalEditsEnabled(JEditorPane editor, boolean enabled) {
		...
		if (log.isDebugEnabled()) {
			log.debug("setLocalEditsEnabled():" + " isEnabled = " + isEnabled + " param = " + enabled);
		}
	...
	public void hasNonConflictingChanges(boolean b) {
		if (log.isDebugEnabled()) {
			log.debug("hasNonConflictingChanges(): _hasNonConflictingChanges = " + _hasNonConflictingChanges + " param = " + b);
		}

		if (_hasNonConflictingChanges != null && _hasNonConflictingChanges.booleanValue() == b) {
			return;
		}
		...
		firePropertyChange(HAS_NON_CONFLICTING_CHANGES_PROPERTY_NAME, oldValue, _hasNonConflictingChanges);
	}
	...
	public void setSelectedStyle(String style) {
		...
		if (_selectedStyle == style || (_selectedStyle != null && _selectedStyle.equals(style))) {
			return;
		}
	...
	public void setStyleSheet(StyleSheet styleSheet) {
		if (log.isDebugEnabled()) {
			...
			log.debug("setStyleSheet(): _styleSheet = " + s1 + " styleSheet param = " + s2);
		}
	...
	public void setCutEnabled(boolean enabled) {
		...
		firePropertyChange(CUT_ENABLED_PROPERTY_NAME, Boolean.valueOf(oldValue), Boolean.valueOf(enabled));
	}
	...
	public void setCopyEnabled(boolean enabled) {
		...
		firePropertyChange(COPY_ENABLED_PROPERTY_NAME, Boolean.valueOf(oldValue), Boolean.valueOf(enabled));
	}
	...
	public void setPasteEnabled(boolean enabled) {
		...
		firePropertyChange(PASTE_ENABLED_PROPERTY_NAME, Boolean.valueOf(oldValue), Boolean.valueOf(enabled));
	}
	...
	public void setRevisionSelected(boolean selected) {
		if (log.isDebugEnabled()) {
			log.debug("setRevisionSelected(): _isRevisionSelected = " + _isRevisionSelected + " selected param = " + selected);
		}
		...
		firePropertyChange(REVISION_SELECTED_PROPERTY_NAME, Boolean.valueOf(oldValue), Boolean.valueOf(selected));
	}
	...
	public void setRemoteRevisionInPara(boolean b) {
		if (log.isDebugEnabled()) {
			log.debug("setRemoteRevisionInPara(): _isRemoteRevisionInPara = " + _isRemoteRevisionInPara + " param = " + b);
		}
		...
		firePropertyChange(REMOTE_REVISION_IN_PARA_PROPERTY_NAME, Boolean.valueOf(oldValue), Boolean.valueOf(b));
	}
	...
	public void setAlignment(int alignment) {
		...
		if (_alignment == alignment || alignment < StyleConstants.ALIGN_LEFT || alignment > StyleConstants.ALIGN_JUSTIFIED) {
			return;
		}
		int oldValue = _alignment;
		_alignment = alignment;
		firePropertyChange(ALIGNMENT_PROPERTY_NAME, Integer.valueOf(oldValue), Integer.valueOf(alignment));
	}
	...
	public void setFontFamily(String fontFamily) {
		...
		if (_fontFamily == fontFamily || (_fontFamily != null && _fontFamily.equals(fontFamily))) {
			return;
		}
	...
	public void setFontSize(int fontSize) {
		...
		_fontSize = fontSize;
		firePropertyChange(FONT_SIZE_PROPERTY_NAME, Integer.toString(oldValue), Integer.toString(fontSize));
	}
	...
	private void setCurrentEditor(JEditorPane editor) {
		...
		if (iframe != null) {
			...
			setDocumentDirty(editor, newDirty);
		}
	...
	/**
	 * Removes a PropertyChangeListener from the listener list. This method should be used to remove the PropertyChangeListeners that were
	 ...
	 *
	 * @return all of this ToolBarStates's <code>PropertyChangeListener</code>s or an empty array if no property change listeners are
	...
	/**
	 * Adds a PropertyChangeListener to the listener list for a specific property. If listener is null, no exception is thrown and no action
	 * is performed.
	 *
	 * @param propertyName
	 ...
	 * 
	 * @return all of the <code>PropertyChangeListener</code>s associated with the named property or an empty array if no such listeners
	 *         have been added.
	 * 
	 * @see #addPropertyChangeListener(java.lang.String,java.beans.PropertyChangeListener)
	 * @see #removePropertyChangeListener(java.lang.String,java.beans.PropertyChangeListener)
	...
	/**
	 * Fires a PropertyChangeEvent in response to a change in a bound property. The event will be delivered to all registered
	 * PropertyChangeListeners. No event will be delivered if oldValue and newValue are the same.
	 *
	 * @param propertyName
	...
	private void setFormatInfo(JEditorPane editor) {
		...
		MutableAttributeSet inputAttrs = ((WordMLTextPane) editor).getInputAttributesML();
		if (inputAttrs.isDefined(WordMLStyleConstants.RStyleAttribute)) {
			// user has selected a style
			String styleName = (String) inputAttrs.getAttribute(WordMLStyleConstants.RStyleAttribute);
			if (styleName != null) {
	...
	private void setDefaultFormatInfo() {
		...
		String styleName = (String) style.getAttribute(WordMLStyleConstants.DefaultParagraphStyleNameAttribute);
		style = styleSheet.getReferredStyle(styleName);

		setFormatInfo(styleSheet, styleName, style);
	}

	private void setFormatInfo(StyleSheet styleSheet, String selectedStyleName, AttributeSet attrs) {
	...
	private JInternalFrame getInternalFrame(JEditorPane editor) {
		return (JInternalFrame) SwingUtilities.getAncestorOfClass(JInternalFrame.class, editor);
	}
	...
	private void setRemoteRevision(WordMLTextPane editor) {
		...
		hasNonConflictingChanges((client == null) ? false : client.hasNonConflictingChanges());
	}
	...
	public void focusGained(FocusEvent e) {
		...
		firePropertyChange(CARET_UPDATE_PROPERTY_NAME, null, new CaretEventImpl((JEditorPane) e.getSource()));
	}
	...
	/**
	 * Gives notification that there was an insert into the document. The range given by the DocumentEvent bounds the freshly inserted
	 * region.
	 *
	 * @param e
	 *            the document event
	...
	public void insertUpdate(DocumentEvent e) {
		...
		if (_currentEditor != null && _currentEditor.getDocument() == e.getDocument()) {
	...
	/**
	 * Gives notification that a portion of the document has been removed. The range is given in terms of what the view last saw (that is,
	...
	public void internalFrameClosed(InternalFrameEvent e) {
		...
		if (_iframeNumbers == 0) {
			...
			firePropertyChange(CARET_UPDATE_PROPERTY_NAME, new CaretEventImpl(SwingUtil.getWordMLTextPane(iframe)), null);
		}
	}
	...
	public void caretUpdate(CaretEvent e) {
		if (_currentEditor == e.getSource()) {
			...
			if (_currentEditor instanceof WordMLTextPane) {
				...
				hasSelection = hasSelection && start == DocUtil.getRevisionStart(doc, start, SwingConstants.NEXT)
