...
public class WordMLApplet extends JApplet {
	...
	public void init() {
		...
		if (s == null) {
			...
			} else if (s.startsWith("http://") || s.startsWith("webdav://")) {
				final String urlParam = s;
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
	...
	private void openRemoteFile(WordMLEditor editor, String urlParam) {
		...
		HyperlinkMenu.getInstance().openWebdavDocument(webdavUrl, recordAsLastOpenUrl, createNewIfNotFound, newPackage, callerActionName,
				null); // TODO: fix this (null value) !!!
	}
