...
public class ContentControlMenu extends UIMenu {
	...
	/**
	 * The binding key used for this ContentControlMenu object when passed into scripting environment
	 */
	...
	private static final String[] _menuItemActionNames = { CHANGE_INTO_SDT_ACTION_NAME, REMOVE_SDT_ACTION_NAME,
			INSERT_EMPTY_SDT_ACTION_NAME, SEPARATOR_CODE, MERGE_SDT_ACTION_NAME, SPLIT_SDT_ACTION_NAME, SEPARATOR_CODE,
			SETUP_SDT_ACTION_NAME, CLEAN_SDT_ACTION_NAME };
	...
	protected JMenuItem createMenuItem(String actionName) {
		...
		if (CHANGE_INTO_SDT_ACTION_NAME.equals(actionName)) {
			...
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new ChangeIntoSdtEnabler(theItem));
		} else if (REMOVE_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new RemoveSdtEnabler(theItem));
		} else if (INSERT_EMPTY_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new InsertEmptySdtEnabler(theItem));
		} else if (MERGE_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new MergeSdtEnabler(theItem));
		} else if (SPLIT_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new SplitSdtEnabler(theItem));
		} else if (SETUP_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new SetupSdtEnabler(theItem));
		} else if (CLEAN_SDT_ACTION_NAME.equals(actionName)) {
			theItem.setEnabled(false);
			toolbarStates.addPropertyChangeListener(ToolBarStates.CARET_UPDATE_PROPERTY_NAME, new CleanSdtEnabler(theItem));
		}
	...
	@Action
	public void changeIntoSdt(ActionEvent evt) {
		WordMLEditorKit.ChangeIntoSdtAction action = new WordMLEditorKit.ChangeIntoSdtAction();
		action.actionPerformed(evt);
	}

	@Action
	public void removeSdt(ActionEvent evt) {
		WordMLEditorKit.RemoveSdtAction action = new WordMLEditorKit.RemoveSdtAction();
		action.actionPerformed(evt);
	}

	@Action
	public void insertEmptySdt(ActionEvent evt) {
		WordMLEditorKit.InsertEmptySdtAction action = new WordMLEditorKit.InsertEmptySdtAction();
		action.actionPerformed(evt);
		if (!action.success()) {
			WordMLEditor editor = WordMLEditor.getInstance(WordMLEditor.class);
			ResourceMap rm = editor.getContext().getResourceMap(getClass());
			String title = rm.getString(INSERT_EMPTY_SDT_ACTION_NAME + ".Action.text");
			String message = rm.getString(INSERT_EMPTY_SDT_ACTION_NAME + ".Action.failureMessage");
			editor.showMessageDialog(title, message, JOptionPane.INFORMATION_MESSAGE);
		}
	}

	@Action
	public void mergeSdt(ActionEvent evt) {
		WordMLEditorKit.MergeSdtAction action = new WordMLEditorKit.MergeSdtAction();
		action.actionPerformed(evt);
	}

	@Action
	public void splitSdt(ActionEvent evt) {
		WordMLEditorKit.SplitSdtAction action = new WordMLEditorKit.SplitSdtAction();
		action.actionPerformed(evt);
	}

	@Action
	public void setupSdt(ActionEvent evt) {
		WordMLEditor editor = WordMLEditor.getInstance(WordMLEditor.class);
		ResourceMap rm = editor.getContext().getResourceMap(getClass());
		String title = rm.getString(SETUP_SDT_ACTION_NAME + ".Action.text");
		...
		ContentGroupingDialog d = new ContentGroupingDialog(editor, DocUtil.getDefinedParagraphStyles(doc));
		d.pack();
		...
		if (d.getValue() == ContentGroupingDialog.OK_BUTTON_TEXT) {
			if (d.isGroupOnEachParagraph()) {
				WordMLEditorKit.CreateSdtOnEachParaAction action = new WordMLEditorKit.CreateSdtOnEachParaAction();
				action.actionPerformed(evt);

			} else if (d.isGroupOnStyles()) {
				List<Integer> positions = DocUtil.getOffsetsOfStyledParagraphs(doc, d.getSelectedStyles());
				if (positions == null) {
					String message = rm.getString(SETUP_SDT_ACTION_NAME + ".Action.noStylesMessage");
					editor.showMessageDialog(title, message, JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				WordMLEditorKit.CreateSdtOnStylesAction action = new WordMLEditorKit.CreateSdtOnStylesAction(positions,
				...
				if (positions == null) {
					String message = rm.getString(SETUP_SDT_ACTION_NAME + ".Action.noSignatureMessage");
					editor.showMessageDialog(title, message, JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				WordMLEditorKit.CreateSdtOnSignedParaAction action = new WordMLEditorKit.CreateSdtOnSignedParaAction(positions, false);
				action.actionPerformed(evt);
			}
		}
	} // setupSdt()
	...
	public void cleanSdt() {
		...
		try {
			...
			WordprocessingMLPackage wmlPackage = XmlUtil.export(docML.getWordprocessingMLPackage(), doc.getElementMLFactory()
					.getObjectFactory());
			DocumentML newDocML = new DocumentML(wmlPackage, doc.getElementMLFactory());
	...
	private static class ChangeIntoSdtEnabler extends CaretUpdateEnabler {
		...
		protected boolean isMenuEnabled(CaretEvent caretEvent) {
			...
			if (caretEvent != null) {
				...
				if (source != null && source == wmlEditor.getView(wmlEditor.getEditorViewTabTitle())
	...
	private static class SetupSdtEnabler extends CaretUpdateEnabler {
		...
		protected boolean isMenuEnabled(CaretEvent caretEvent) {
			...
			if (caretEvent != null) {
				WordMLEditor wmlEditor = WordMLEditor.getInstance(WordMLEditor.class);
				JEditorPane source = (JEditorPane) caretEvent.getSource();
				JEditorPane editorView = wmlEditor.getView(wmlEditor.getEditorViewTabTitle());
